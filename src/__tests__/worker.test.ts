...
describe('Cloudflare Worker entrypoint', () => {
  ...
  it('handles MCP initialize', async () => {
    ...
    expect(body.result?.serverInfo?.name).toBe('hudu-mcp');
  });
  ...
  it('lists all tools without credentials', async () => {
    ...
    expect(names).toContain('hudu_test_connection');
    expect(names).toContain('hudu_list_companies');
  ...
  it('returns a graceful error for a credential-requiring tool when unconfigured', async () => {
    const res = await mcp({
      ...
      id: 3,
      method: 'tools/call',
      params: { name: 'hudu_list_companies', arguments: {} },
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
